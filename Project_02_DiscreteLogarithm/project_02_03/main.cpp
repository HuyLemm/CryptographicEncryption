...
bignum modularInverse(bignum a, bignum p) {
    return Power_mod(a, p - 2, p);
}

void encrypt(bignum p, bignum g, bignum h, bignum m, bignum& c1, bignum& c2) {
    bignum y, s;
    y = rand();
    y = y % (p - 1) + 1;
    s = Power_mod(h, y, p);
...
void decrypt(bignum p, bignum g, bignum x, bignum c1, bignum c2, bignum& m) {
    bignum s, inv_s;
    s = Power_mod(c1, x, p);
    inv_s = modularInverse(s, p);
    m = (c2 * inv_s) % p;
}
...
int main(int argc, char* argv[]) {
    ...
    h = Power_mod(g,x,p);
    outputFile << h.toHexLittleEndian() << endl;

    decrypt(p,g,x,c1,c2,m);
    outputFile << m.toHexLittleEndian() << endl;
