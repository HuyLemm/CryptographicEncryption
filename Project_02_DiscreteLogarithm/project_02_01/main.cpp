...
bool isPrimitiveRoot(bignum& p, bignum& g, bignum* U, bignum& n) {
    for (bignum i(0); i < n; i = i + 1) {
        const bignum& k = U[i.toInt()];
        bignum exp = (p - 1) / k;
        bignum result = Power_mod(g, exp, p);
        if (result == 1)
            return false;
    }
    return true;
...
int main(int argc, char *argv[])
{
    ...
    bignum* U = new bignum[n.toInt()]; 
    for (int i = 0; i < n.toInt(); i++)
    {
        inputFile >> hex;
        string reversedHexTemp = reverseHex(hex);
        string binTemp = hexToBin(reversedHexTemp);
        bignum temp(binTemp);
        U[i] = bignum(temp);
    }
    ...
    if (isPrimitiveRoot(p, g, U, n) == true)
        outputFile << "1" << endl;
    else
        outputFile << "0" << endl;
