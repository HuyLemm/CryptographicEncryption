...
bignum modularInverse(bignum a, bignum b) {
    return Power_mod(a, b - 2, b);
}

bool verifySignature(bignum p, bignum g, bignum y, bignum m, bignum r, bignum h) {
    if (r <= 0 || r >= p || h <= 0 || h >= p - 1)
        return false;

    bignum a = Power_mod(g, m, p);
    bignum b = (Power_mod(y, r, p) * Power_mod(r, h, p)) % p;
...
int main(int argc, char* argv[]) {
    ...
    getline(inputFile,hex);
    string reversedHex5 = reverseHex(hex);
    string bin5 = hexToBin(reversedHex5);
    bignum r(bin5);

    getline(inputFile,hex);
    string reversedHex6 = reverseHex(hex);
    string bin6 = hexToBin(reversedHex6);
    bignum h(bin6);

    if (verifySignature(p,g,y,m,r,h) == true)
        outputFile << "1" << endl;
    else
        outputFile << "0" << endl;
