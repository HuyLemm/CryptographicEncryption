...
bignum Power_mod(bignum x, bignum y, bignum p)
{
    ...
    while (y > 0)
    {
        if (y.getBit(0)) // Kiểm tra bit cuối cùng của y. Nếu bit này là 1, thực hiện bước nhân.
        {
            result *= x;
            result %= p;
        }
        y >>= 1; // Dịch bit y sang phải (chia y cho 2).
        x *= x;
        x %= p;
...
bool isPrimeMillerRabin(bignum& n, int k) {
    if (n <= 1) 
        return false;
    if (n == 2 || n == 3) 
        return true;
    ...
    srand(time(nullptr));
    for (int i = 0; i < k; ++i) {
        bignum a = bignum(rand()) % (n - 3) + 2; 
        bignum x = Power_mod(a, d, n);
        if (x == 1 || x == n - 1) 
            continue;
        bool isWitness = true;
        for (int r = 0; r < s; ++r) {
...
int main(int argc, char* argv[]) {
    ...
    string bin = hexToBin(reversedHex);
    bignum a(bin);

    if (isPrimeMillerRabin(a, 10) == true)
        outputFile << "1";
    else
        outputFile << "0";
