...
bignum modPow(bignum a, bignum b, bignum m) {
    bignum result(1);
    a %= m;
    while (b > 0) {
        if (b % 2 == 1)
...
bignum findY(bignum x, bignum k, bignum N) {
    bignum y(1);
    for (bignum i(0); i < N; i += 1) {
        if (modPow(x, k, N) == y)
            return y;
        y += 1;
    }
    return -1; // Không tìm thấy y thỏa mãn
}
...
int main(int argc, char* argv[])
{
    ...
    bignum result = modPow(x, k, N);

    cout << result.toHexLittleEndian() << endl;
    outputFile << result.toHexLittleEndian();
